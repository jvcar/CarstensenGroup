function [xPhys,HsV,HS,HsS,HV,U]=MinLS(x,maxloop,figNum,U,freedofs,F,neleD,elloc,el_D,passive_usr)
parameters
...
%% PREPARE FILTER for Rmin Solid
iHS = ones(nelx*nely*(2*(ceil(rminS)-1)+1)^2,1);
jHS = ones(size(iHS));
sHS = zeros(size(iHS));
k = 0;
for i1 = 1:nelx
  for j1 = 1:nely
    e1 = (i1-1)*nely+j1;
    for i2 = max(i1-(ceil(rminS)-1),1):min(i1+(ceil(rminS)-1),nelx)
      for j2 = max(j1-(ceil(rminS)-1),1):min(j1+(ceil(rminS)-1),nely)
        e2 = (i2-1)*nely+j2;
        k = k+1;
        iHS(k) = e1;
        jHS(k) = e2;
        sHS(k) = max(0,rminS-sqrt((i1-i2)^2+(j1-j2)^2));
      end
    end
  end
end
HS = sparse(iHS,jHS,sHS);
HsS = sum(HS,2);

%% PREPARE FILTER for Rmin Void
iHV = ones(nelx*nely*(2*(ceil(rminV)-1)+1)^2,1);
jHV = ones(size(iHV));
sHV = zeros(size(iHV));
k = 0;
for i1 = 1:nelx
  for j1 = 1:nely
    e1 = (i1-1)*nely+j1;
    for i2 = max(i1-(ceil(rminV)-1),1):min(i1+(ceil(rminV)-1),nelx)
      for j2 = max(j1-(ceil(rminV)-1),1):min(j1+(ceil(rminV)-1),nely)
        e2 = (i2-1)*nely+j2;
        k = k+1;
        iHV(k) = e1;
        jHV(k) = e2;
        sHV(k) = max(0,rminV-sqrt((i1-i2)^2+(j1-j2)^2));
      end
    end
  end
end
HV = sparse(iHV,jHV,sHV);
HsV = sum(HV,2);
...
% Miu/xTilde - Filtered Phase Variabiles
...
while change > 0.01 && max_loop>loop
  ...
  else % only take sensitivities of elements in design region
        dcA=dc(:);dvA=dv(:); xA=x; % to preserve dc,dv,x
        dcDes=dcA(el_D);dvDes=dvA(el_D);xdes=xA(el_D);
        xdes=xdes'; 
  end
  %% METHOD OF MOVING ASYMPTOTES
  xval=xdes(:); % only design variables
  f0val=c;
  df0dx=dcDes(:);
  ...
  xTildeS(:)=HS*(Ws(:)./HsS); % Filter for solid
  xTildeS=reshape(xTildeS,nely,nelx);
  xTildeV(:)=HV*(Wv(:)./HsV); % Filter for svoid
  xTildeV=reshape(xTildeV,nely,nelx);
  xPhysS=1-exp(-betaHP*xTildeS)+(xTildeS./umax)*exp(-betaHP*umax); %HPM of solid
  xPhysV=1-exp(-betaHP*xTildeV)+(xTildeV./umax)*exp(-betaHP*umax); %HPM of void
  xPhys=(xPhysS+(1-xPhysV))/2; % combine to one density
