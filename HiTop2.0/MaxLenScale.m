function [xPhys]=MaxLenScale(xPhys,Hmax,HsV,HS,HsS,HV,HsVMax,maxloop,indices,figNum,F,U,freedofs,neleD,elloc,el_D,passive_usr)
parameters
...
Wxv=1./(1+exp(-(xmaxLS-(0-step_xv))*alpha_xv))-1./(1+exp(-(xmaxLS-(0+step_xv))*alpha_xv));
...
xTildeXV(:)=Hmax*(Wxv(:)./HsVMax); %demonimator is just void
umax=phiMax;
...
xPhysXV=1-exp(-betaHP*xTildeXV)+(xTildeXV./umax)*exp(-betaHP*umax); %HPM of max - XV
xPhys=(xPhysS+(ones(nely,nelx)-xPhysV))/2; % combine to one density
xPhysS=xPhysS(:); xPhysV=xPhysV(:);xPhys=xPhys(:);xPhysXV=xPhysXV';
xPhys(indices)=(xPhysS(indices)+xPhysXV-xPhysV(indices))/2;
...
while change > 0.01 && maxloop>loop
  ...
  % d Wxv / d phi
  dwxv=(alpha_xv*exp(alpha_xv*(-1*(xmaxLS)-step_xv+0  )))./(1+exp(alpha_xv*(-1*(xmaxLS)-step_xv+0))).^2-(alpha_xv*exp(alpha_xv*(-1*(xmaxLS)+step_xv+0  )))./(1+exp(alpha_xv*(-1*(xmaxLS)+step_xv+0))).^2;
  % d rho / dUs
  dx_s=(betaHP*exp(-betaHP.*xTildeS)+exp(-betaHP)); 
  % drho / dUv
  dx_v=(betaHP*exp(-betaHP.*xTildeV)+exp(-betaHP));
  % drho / dUxv
  dx_xv=(betaHP*exp(-betaHP.*xTildeXV)+exp(-betaHP));
  % dc / dWs (dc/drho * drho/dUs * dUs/dWs)
  dc_s=.5*(HS*(dc(:).*dx_s(:)))./HsS;
  % dc / Wv (dc/drho * drho/dUv * dUv/dWv)
  dc_v=-.5*(HV*(dc(:).*dx_v(:)))./HsV;
  % dc / Wxv (dc/drho * drho/dUxv * dUxv/dWxv)
  dcMax=dc(:);
  dcMax=dcMax(indices);
  dc_xv=.5*(Hmax*(dcMax.*dx_xv(:)))./HsVMax;
  ...
  dc_v=dc_v.*dwv(:);
  dc_v=dc_v(:);
  % dc / dphi (dc/dWxv * dWxv/dphi)
  dc_xv=dc_xv.*dwxv(:);
  dc_xv=dc_xv(:);
  % combine dc 
  dc=(dc_s+dc_v);
  dc(indices)=dc_s(indices)+dc_xv+dc_v(indices);
  % dv / dWs (dv/drho * drho/dUs * dUs/dWs)
  dv_s(:)=.5*(HS*(dv(:).*dx_s(:)))./HsS;
  % dv / dWv (dv/drho * drho/dUv * dUv/dWv)
  dv_v(:)=-.5*(HV*(dv(:).*dx_v(:)))./HsV;
  % dv / dWxv (dv/drho * drho/dUxv * dUxv/dWxv)
  dvMax=dv(:);
  dvMax=dvMax(indices);
  clear dv_xv;
  dv_xv(:)=.5*(Hmax*(dvMax.*dx_xv(:)))./HsVMax;
  dv_xv=dv_xv';
  % dv_s/dphi (dv/dWs * dWs/dphi)
  dv_s=reshape(dv_s,nele,1);
  dv_s=dv_s.*dws(:);
  dv_s=dv_s(:);
  % dv_v/dphi (dv/dWv * dWv/dphi)
  dv_v=reshape(dv_v,nele,1);
  dv_v=dv_v.*dwv(:);
  dv_v=dv_v(:);
  % dv_xv/dphi (dv/dWxv * dWxv/dphi)
  dv_xv=dv_xv.*dwxv;
  dv_xv=dv_xv(:);
  % combine dxv
  dv=dv_s+dv_v;
  dv(indices)=(dv_s(indices)+dv_v(indices)+dv_xv);
  ...
  f0val=c;
  df0dx=dcDes(:);
  fval=sum(xPhys(:))/(volfrac*nele)-1; % rho - filtered, HPM
  ...
  xTildeV=reshape(xTildeV,nely,nelx);
  xTildeXV(:)=Hmax*(Wxv(:)./HsVMax); %demonimator is just void
%   xTildeXV=reshape(xTildeXV,nely,nelx);
  umax=phiMax;
  ...
  elseif loop >=350
    ...
    function graphHP(step_s,step_xv,step_v,alpha,alpha_xv,iter,t)
        ...
        plot(xgraph,ygraph_sO,'r','LineStyle',':');
        plot(xgraph,ygraph_vO,'b','LineStyle',':');
        plot(xgraph,ygraph_xvO,'g','LineStyle',':');
