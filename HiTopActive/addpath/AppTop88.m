function [xPhys,D,A]=AppTop88(xPhys,xPhysbig,Astar,ind,num_ind,max_loop,U,freedofs,F)
parameters
%% MATERIAL PROPERTIES
E0 = 1;
Emin = 1e-9;
...
while change > 0.01 && max_loop>loop
  ...
  dD_drhoTotal=2*(xPhys-xPhysbig);
  dA_drho=dD_drhoTotal(ind)./(Astar*num_ind);
  dA_drhoUSE=zeros(nely,nelx);
  dA_drhoUSE(ind)=dA_drho;
  dA_filt=H*((dA_drhoUSE(:).*dx(:))./Hs); % dA / dx
  ...
  x = xnew;
  %% PRINT RESULTS
  fprintf(' It.:%5i Obj.:%11.4f Vol.:%7.3f ch.:%7.3f p:%7.3f Abig:%7.3f\n',loop,c, ...
    mean(xPhys(:)),change,penal,A*Astar);
  %% PLOT DENSITIES
  figure(4)
  colormap(gray); imagesc(1-xPhys); caxis([0 1]); axis equal; axis off; drawnow;
  if loop > 150 && loop < 300
      penal=5;
      betaHP=15; 
  elseif loop > 300 
      penal=7;
